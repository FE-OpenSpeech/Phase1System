...
%
% This version of register settings creates an Altera .mif
% file for the register settings that are loaded from a ROM
% 
...
%--------------------------------------------------------------------------
% Create an Altera .mif file for a ROM that loads the default
% register settings on power-up
% See the following link for the specifications of the .mif file format:
% http://quartushelp.altera.com/15.0/mergedProjects/reference/glossary/def_mif.htm
%--------------------------------------------------------------------------
filename = 'AD1939_reg_init_ROM_values.mif';
fid = fopen(filename,'w');
%-------------------------------------------------------------------
% Determine size of ROM needed, which needs to be a power of 2
%-------------------------------------------------------------------
Total_Registers        = length(register);
Total_Memory_Words     = 2^ceil(log2(Total_Registers));
Total_Memory_Word_Size = 8;
disp([' Create a ROM (' filename ') that is ' num2str(Total_Memory_Words) ' Words x ' num2str(Total_Memory_Word_Size) ' bits '])

%------------------------------------
% Write Altera .mif File Header
%------------------------------------
line = ['DEPTH = ' num2str(Total_Memory_Words) ';']; fprintf(fid,'%s\n',line); % The size of memory in words
line = ['WIDTH = ' num2str(Total_Memory_Word_Size) ';']; fprintf(fid,'%s\n',line); % The size of data in bits
line = ['ADDRESS_RADIX = DEC;']; fprintf(fid,'%s\n',line); % The radix for address values (it can be binary (BIN), hexadecimal (HEX), octal (OCT), signed decimal (DEC), or unsigned decimal (UNS) )
line = ['DATA_RADIX = BIN;']; fprintf(fid,'%s\n',line); % The radix for data values (radix choices similar to address radix choices)
line = ['CONTENT'];  fprintf(fid,'%s\n',line);
line = ['BEGIN'];  fprintf(fid,'%s\n',line);  % start of memory content, written as (address : data) pairs
%------------------------------------
% Write Memory Data
%------------------------------------
for data_index = 1:Total_Memory_Words
    if data_index <= Total_Registers
        data_string = register{data_index}.data;
        line = [num2str(data_index-1) ' : ' data_string ';   -- AD1939 Register ' num2str(data_index) '(Address=' num2str(data_index-1) ')' ];  fprintf(fid,'%s\n',line);
    else
        data_string = [];
        for k=1:Total_Memory_Word_Size
            data_string = [data_string '0'];  % could have used .bin from fixed-pt toolbox, but decided to not make the assumption that one doesn't have this toolbox
        end
        line = [num2str(data_index-1) ' : ' data_string ';   -- zero rest of memory' ];  fprintf(fid,'%s\n',line);
    end
end
%------------------------------------
% Write File End
%------------------------------------
line = ['END;']; fprintf(fid,'%s\n',line);  % start of (address : data pairs)
%------------------------------------
% Close File
%------------------------------------
fclose(fid);
