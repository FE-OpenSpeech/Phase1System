function v = fxpt_exp_vhdl_code_gen_ROM_lnbp_coef( b )
...
v.component = [];       
entity1   = ['fxpt_exp_ROM_lnbp_coef_W' num2str(W) 'F' num2str(F)]; v.entity = entity1;
...
str = ['--       This VDHL file creates a ROM of bi=ln(1+2^-i) coefficients (lnbp = ln(b_positive)) to be used in']; fprintf(fid,'%s\n',str);
...
str = ['      lnbp_coef  : out std_logic_vector(' num2str(W-1) ' downto 0)']; fprintf(fid,'%s\n',str); v.component = char(v.component,str);
str = ['   );']; fprintf(fid,'%s\n',str); v.component = char(v.component,str);
...
for i=0:F-1
    awf = fi(i,0,AW,0);
    ob = b(i+1);
    str = ['              when "' awf.bin '" => lnbp_coef <= "' ob.bin '";  -- bi ='  fxpt2str(ob)]; fprintf(fid,'%s\n',str);
end
str = ['              when others  => lnbp_coef <= (others => ''0'');']; fprintf(fid,'%s\n',str);
