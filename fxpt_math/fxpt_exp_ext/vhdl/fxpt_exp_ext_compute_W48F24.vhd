...
entity fxpt_exp_ext_compute_W48F24 is
   port (
      clock	: in  std_logic;
      reset	: in  std_logic;
      x	    : in  std_logic_vector(47 downto 0);
      start	: in  std_logic;
      y     : out std_logic_vector(47 downto 0);   -- y=exp(x)  (extended x domain)
      done  : out std_logic
   );
end entity;

architecture rtl of fxpt_exp_ext_compute_W48F24 is
   ...
   constant log2e           :   signed(47 downto 0) := "000000000000000000000001011100010101010001110110";  --000001715476
   constant ln2             :   signed(47 downto 0) := "000000000000000000000000101100010111001000010111";  --000000b17217
   signal exp_flag_start    :   std_logic;
   signal exp_flag_done     :   std_logic;
   signal t1                :   signed(47 downto 0);
   signal t1_abs            :   unsigned(47 downto 0);
   signal t1I               :   unsigned(23 downto 0);
   signal t1F               :   signed(47 downto 0);
   signal t2                :   signed(47 downto 0);
   signal t2_resized        :   std_logic_vector(31 downto 0);
   signal t3                :   signed(47 downto 0);
   signal t4                :   signed(47 downto 0);
   signal y2                :   std_logic_vector(31 downto 0);
   signal y2_resized        :   signed(47 downto 0);
   signal yo                :   signed(47 downto 0);
...
begin
  ...
  EXP1 : fxpt_exp_compute_W32F28
   ...
   t2_resized <= std_logic_vector( t2(27 downto 0) & "0000" );        -- resize signal to match exp() signal I/O
   y2_resized <= resize(signed(y2(31 downto 4)), y2_resized'length);   -- resize signal to match exp() signal I/O
   ...
   compute : process (clock)
      variable t1v   :   signed(95 downto 0);
      variable t2v   :   signed(95 downto 0);
   begin
         if (rising_edge(clock)) then
               ...
               case state is
                     ...
                     when state_c3 =>
                           t1I <= t1_abs(47 downto 24);   -- Integer bits 
                           if t1(47) = '0' then
                                 t1F <= signed("000000000000000000000000" & t1_abs(23 downto 0));    -- Fractional bits
                           else
                                 t1F <= signed(not("000000000000000000000000" & t1_abs(23 downto 0))+1);    -- Fractional bits
                           end if;
