function v = fxpt_log_vhdl_code_gen_ROM_b_coef( b )
...
v.component = [];    
entity1   = ['fxpt_log_ROM_b_coef_W' num2str(W) 'F' num2str(F)]; v.entity = entity1;
...
str = ['--       This VDHL file creates a ROM of bi=(1+2^-i) coefficients to be used in']; fprintf(fid,'%s\n',str);
...
str = ['      b_coef    : out std_logic_vector(' num2str(W-1) ' downto 0)']; fprintf(fid,'%s\n',str); v.component = char(v.component,str);
str = ['   );']; fprintf(fid,'%s\n',str); v.component = char(v.component,str);
...
for i=0:W-1
    awf = fi(i,0,AW,0);
    ob = b(i+1);
    str = ['              when "' awf.bin '" => b_coef <= "' ob.bin '";  -- bi = '  fxpt2str(ob) ' = ' ob.hex]; fprintf(fid,'%s\n',str);
end
str = ['              when others  => b_coef <= (others => ''0'');']; fprintf(fid,'%s\n',str);
