...
This is a linux module driver to control the output mux selection.

To use it in the terminal commands can be "echo 1 > /dev/fe_omux"'ed into the device
...
@verbatim
The outputs are:
0 - All channels muted     (0b00)
1 - Line Out selected      (0b01)
2 - Headphone Out selected (0b10)
3 - Both outputs selected  (0b11)
@endverbatim
*/
...
MODULE_DESCRIPTION("Loadable kernel module for the FE QSys Output Mux block");
MODULE_VERSION("1.0");
...
static DEVICE_ATTR(output, 0664, output_show, output_store);        // Create a device attribute to be able to set in /sys/class
...
static struct platform_driver mux_platform = {
    ...
    .remove = mux_remove,
    .driver = {
        .name = "Flat Earth Output Mux Driver",
        .owner = THIS_MODULE,
...
{
    int ret_val = 0;
    pr_info("Initializing the Flat Earth Output Mux module\n");
    ...
    pr_info("Flat Earth Output Mux module successfully initialized!\n");
...
static int mux_probe(struct platform_device *pdev)
{
    ...
    fe_mux_devp->mux_value = 0x0003;
    iowrite32(fe_mux_devp->mux_value, (u32*)fe_mux_devp->regs + OUTPUT_MODE_ADDR);
    ...
    status = alloc_chrdev_region(&dev_num, 0, 1, "fe_omux");
    if (status != 0)
    ...
    status = device_create_file(deviceObj, &dev_attr_output);
    if (status)
...
bad_device_create_file_2:
    device_remove_file(deviceObj, &dev_attr_output);
...
{
    pr_info("Flat Earth Output Mux module exit\n");
    ...
    pr_info("Flat Earth Output Mux module successfully unregistered\n");
}


/** Function to read the output mux setting

Valid outputs are "mute","output1","output2", "both", or "invalid"

@param dev Pointer to the instance of the device driver
@param attr ?
@param buf Output of this command
...
/** Function to store the output mux setting

Funtion to be able to store a value in the output mux in the sysfs class directory
Valid modes are are ["mute" or "0"], ["output1" or "1"], ["output2" or "2"], and ["both" or "3"]
...
static ssize_t output_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (strcmp(buf, "mute") || strcmp(buf, "0"))
        devp->mux_value = 0;
    else if (strcmp(buf, "output1") || strcmp(buf, "1"))
        devp->mux_value = 1;
    else if (strcmp(buf, "output2") || strcmp(buf, "2"))
        devp->mux_value = 2;
    else if (strcmp(buf, "both") || strcmp(buf, "3"))
        devp->mux_value = 3;
    else
    ...
    iowrite32(devp->mux_value, (u32*)devp->regs + OUTPUT_MODE_ADDR);
